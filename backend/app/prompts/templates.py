...
1. **ZERO TOLERANCE FOR HALLUCINATION**: If a metric (e.g., %, $, time) is NOT in the original resume, DO NOT invent it. 
...
3. **THREE-LINE SUMMARY STRUCTURE**:
   - Line 1 (Identity): Who they are + Education (e.g., 'IT Student at Bansal Institute').
   - Line 2 (Core Arsenal): Top 4-5 hard skills + strongest certification.
   - Line 3 (Direction): Professional focus/goal.
4. **ACTION + TECH + OUTCOME**: For projects, describe the technical "How" and the functional "Why" using varied action verbs.
...
OUTPUT (Start with the word SUMMARY):
"""